...
| 使用场景 | 画布提供什么 |
|---|---|
| **AI 漫剧 / 动态漫** | 一集一个分区：角色与场景资产块跨镜头复用，一个共享的美术风格节点向下游所有提示词供词，让画风在几十格里保持一致 |
| **AI 微电影** | 剧本 → 分镜 → 镜头 → 剪辑分区让整片状态一眼可查；首尾帧关键帧串联让相邻镜头真正衔接，而不是 PPT 切片 |
| **AI 视频创作** | 任何多镜头作品——商品广告、MV、讲解片。节点级 `canvas_refresh_node` 只重生成没做好的那一个镜头，不必重做整块画布 |

下面这些能力让以上场景可行：每项目一套**素材库**（人物 / 场景 / 音频 / 视频片段）保证主角形象稳定；**软引用**让一个角色资产服务多张画布而无需复制文件；**全局搜索（⌘K）**能从旧项目里找到场景资产直接复用到当前画布。
